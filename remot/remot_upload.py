...
if __name__=="__main__":
    remote_path = '/usr/local/Tengine_nginx20160125.tar.gz'
    local_path = '/usr/local/Tengine_nginx20160125.tar.gz'
    f_size = os.path.getsize(local_path)
    cmd = ['cd /usr/local/ ; \
			tar -zxvf Tengine_nginx20160125.tar.gz ; ',
			'groupadd -r nginx ; \
			useradd -s /sbin/lologin -g nginx -r nginx ;',
			'mkdir /data ;',
			'mv /usr/local/libluajit-5.1.so.2 /usr/lib64/ ;',
			'mv /usr/local/libpcre.so.0 /lib64/ ;',
			'mv /usr/local/libgd.so.2 /usr/lib64/libgd.so.2',
